...
public class cruise {
    ...
    public void setCruiseRoute(String cruiseRoute){
        if(cruiseRoute.isEmpty() || cruiseRoute == null){
            throw new IllegalArgumentException("Cruise route cannot be empty. Enter a cruise route in the format: " +
    ...
    public static void setCruiseCount(int cruiseCount){
        ...
        try{
            ...
            JOptionPane.showMessageDialog(null, "Cruise count must be an integer.");
        }
    }

    public void setTicketCost(double ticketCost){
        if(ticketCost < 0.01){
            throw new IllegalArgumentException("Ticket cost cannot be less than $0.01.");
        }
        try{
            this.ticketCost = ticketCost;           
        }catch(NumberFormatException nfe){
            JOptionPane.showMessageDialog(null, "Ticket cost must be a valid decimal number.");
        }
    }

    public void setTicketSoldCount(int ticketSoldCount){
        if(ticketSoldCount < 1){
            throw new IllegalArgumentException("Ticket sold count cannot be less than 1.");
        }
        try{
            this.ticketSoldCount = ticketSoldCount;
        }catch(NumberFormatException nfe){
            JOptionPane.showMessageDialog(null, "Ticket sold count must be a valid integer.");
        }
    }

    public static void setTotalTicketSoldCount(int totalTicketSoldCount){
        if(totalTicketSoldCount < 0){
            throw new IllegalArgumentException("Total ticket sold count a cannot be less than 0.");
        }
        try{
            cruise.totalTicketSoldCount = totalTicketSoldCount;
        }catch(NumberFormatException nfe){
            JOptionPane.showMessageDialog(null, "Total ticket sold count must be a valid integer.");
        }
    ...
    public static void setTotalEarning(double totalEarning){
        if(totalEarning < 0){
            throw new IllegalArgumentException("Total earning must be positive");
        }
        try{
